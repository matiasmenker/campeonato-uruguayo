...
availabilityRouter.get("/api/v1/player-market-values", async (request, response, next) => {
  try {
...
availabilityRouter.get("/api/v1/players/:id/market-values", async (request, response, next) => {
  try {
...
availabilityRouter.get("/api/v1/injuries", async (request, response, next) => {
  try {
...
availabilityRouter.get("/api/v1/players/:id/injuries", async (request, response, next) => {
  try {
    const { id } = playerIdParamSchema.parse(request.params);
    const query = injuriesQuerySchema.parse(request.query);
...
availabilityRouter.get("/api/v1/suspensions", async (request, response, next) => {
  try {
...
availabilityRouter.get("/api/v1/players/:id/suspensions", async (request, response, next) => {
  try {
    const { id } = playerIdParamSchema.parse(request.params);
    const query = suspensionsQuerySchema.parse(request.query);
