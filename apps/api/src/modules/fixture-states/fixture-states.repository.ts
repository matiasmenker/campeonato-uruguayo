...
export const findFixtureStates = async (
...
export const findFixtureStateById = async (id: number): Promise<FixtureState | null> => {
  const prisma = getPrisma();
  return prisma.fixtureState.findUnique({ where: { id } });
};
export const findFixtureStatesByIds = async (ids: number[]): Promise<Map<number, FixtureState>> => {
