 ...
 *  - Standings & coaches
 */
export async function syncHealth(dependencies: SyncDependencies): Promise<void> {
  ...
  const apiSeasonDetail = await client.get<{
    stages?: { data: Array<{ id: number }> } | Array<{ id: number }>;
    rounds?: { data: Array<{ id: number }> } | Array<{ id: number }>;
  ...
  const teamPlayerMismatches: Array<{ team: string; database: number; api: number }> = [];
